...
def predict(user_input):
  ...
  if ham_counter > spam_counter:
    ...
    return 'message is not spam, with {}% accuracy'.format(accuracy), green
  elif spam_counter > ham_counter:
    accuracy = round((spam_counter / (ham_counter + spam_counter)) * 100, 2)
    return 'message is spam, with {}% accuracy'.format(accuracy), red
  else:
    return 'message could be spam, with 50% accuracy', [255,255,255]
